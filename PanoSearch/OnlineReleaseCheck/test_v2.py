...
def myRqeuest():
    for i in range(100):
        ...
        content = r.json()
        dataItem = content["data"]["home_page_module_order"][4]["records"][0]
        if dataItem["content"] != "找工作" and "&package=&" in dataItem["action"]:
            print("第%i次请求，没有应用新数据"% i)

def myThread():
    for i in range(50):
        t = threading.Thread(target=myRqeuest)
        t.start()
